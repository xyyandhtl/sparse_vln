...
class VLMapBuilderSfMSparse:
    def __init__(
        ...
        self.gravity_z = map_config.gravity_axis == 'z'
    ...
    def create_camera_map_sfm(self):
        ...
        print(f'sparse images {len(self.sfm_images)}, points {len(self.points3d_ordered)}, '
              f'reproj {self.mean_reproj_error}, track {self.mean_track_length}')
        ...
        for sfm_image in self.sfm_images.values():
            ...
            if self.gravity_z:
                ...
                sum_height += sfm_pos[2]
            else:
                max_y = max(abs(sfm_pos[2]), max_y)
                sum_height += sfm_pos[1]
        ave_height = round(sum_height / len(self.sfm_images))
        self.pcd_min = np.array([-max_x * expand_ratio, ave_height - self.map_config.camera_height, -max_y * expand_ratio])
        self.pcd_max = np.array([max_x * expand_ratio, ave_height + self.map_config.camera_height, max_y * expand_ratio])
        ...
        for frame_i, (sfm_image, camera_pose_tf) in enumerate(pbar):
            ...
            # filter invalid
            pts_idx = sfm_image.point3D_ids
            mask = pts_idx >= 0
            mask *= pts_idx < len(self.points3d_ordered)
            pts_idx = pts_idx[mask]

            # filter unreliable
            mask = np.array([len(self.sfm_points[idx].image_ids) for idx in pts_idx]) >= self.mean_track_length
            mask *= np.array([self.sfm_points[idx].error for idx in pts_idx]) <= self.mean_reproj_error + 0.5
            pts_idx = pts_idx[mask]
            # valid_xys = sfm_image.xys[mask]

            if len(pts_idx) == 0:
                continue

            pts = self.points3d_ordered[pts_idx]
            pc_global = pts.transpose()

            rot = R.from_quat(np.roll(sfm_image.qvec, -1)).as_matrix()
            pts = np.dot(pts, rot.T) + sfm_image.tvec
            pc = pts.transpose()
            ...
            # print(f'pix_feats shape {pix_feats.shape}')
            ...
            for i, (p, p_local) in enumerate(zip(pc_global.T, pc.T)):
                if self.gravity_z:
                    row, height, col = np.round(((np.array([p[0], p[2], p[1]]) - self.pcd_min) / cs)).astype(int)
                else:
                    row, height, col = np.round(((p - self.pcd_min) / cs)).astype(int)
                # print(f'row, col, height: {row}, {col}, {height}')
                ...
                if radial_dist_sq > 2500:  # distance from camera > 50m.  original: alpha == 0.0:
                    # print(f"point too far! alpha/radial_dist_sq/sum {alpha}/{radial_dist_sq}/{np.square(p_local)}")
                    continue  # too far from cam would cause alpha --> 0
                ...
                if px >= 0 and py >= 0 and px < pix_feats.shape[3] and py < pix_feats.shape[2]:
                    ...
                    if occupied_id == -1:
                        ...
                        weight[occupied_id] += alpha
                    # print(f'updating r/h/c {row}/{height}/{col} grid with occupied_id {occupied_id}, '
                    #       f'alpha {alpha}, weight {weight[occupied_id]}')

            print(f'{sfm_image.name} applying {max_id - count_init}/{len(pts_idx)}/{len(sfm_image.point3D_ids)} '
                  f'inbox/reliable/total 3d points')
    ...
    def _init_map(self, pcd_min: np.ndarray, pcd_max: np.ndarray, cs: float, map_path: Path) -> Tuple:
        ...
        """
        # replace np.ceil with np.floor to syns with grid_map builder
        grid_size = np.floor((pcd_max - pcd_min) / cs + 1).astype(int)  # col, height, row
    ...
    def _init_lseg(self):
        ...
        if torch.cuda.is_available():
            ...
            self.device = "cpu"
        print(f'lseg using device {self.device}')
    ...
    def save_3d_map(
        ...
        print(f'grid occupied status: {max_id}/{grid_feat.shape[0]}')
        ...
        rgb_2d = np.zeros((occupied_ids.shape[0], occupied_ids.shape[2], 3), dtype=np.uint8)
        height = -100 * np.ones((occupied_ids.shape[0], occupied_ids.shape[2]), dtype=np.int32)
        for i, pos in enumerate(grid_pos):
            # row, col, h = pos
            row, h, col = pos
            if h > height[row, col]:
                rgb_2d[row, col] = grid_rgb[i]
                height[row, col] = h
        cv2.imwrite(str(self.map_rgb_path), np.transpose(rgb_2d, (1, 0, 2)))
