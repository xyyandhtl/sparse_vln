...
from ..utils.visualize_utils import pool_3d_label_to_2d

from .vlmap_builder_sparse import VLMapBuilderSfMSparse
from .vlmap_builder_dense import VLMapBuilderSfMDense
# from vlmaps.map.vlmap_builder_cam import VLMapBuilderCam
from ..utils.mapping_utils import load_3d_map
from ..utils.index_utils import find_similar_category_id, get_segment_islands_pos, get_dynamic_obstacles_map_3d
from ..utils.clip_utils import get_lseg_score
from .map import Map
...
class VLMap(Map):
    ...
    def create_map(self, data_dir: Union[Path, str]) -> None:
        ...
        self.map_builder = VLMapBuilderSfMSparse(
            self.data_dir,
            self.map_config,
            self.sfm_dir,
            self.rgb_dir,
        ...
        self.map_builder.create_camera_map_sfm()
    ...
    def load_map(self, data_dir: str) -> bool:
        ...
        (
            ...
            self.pcd_max,
            self.cs,
        ) = VLMapBuilderSfMSparse.load_3d_map(self.map_save_path)
    ...
    def index_map(self, language_desc: str, with_init_cat: bool = True):
        if with_init_cat and self.scores_mat is not None and self.categories is not None:
            ...
            print(f'using init_cat id {cat_id}')
        else:
            if with_init_cat:
                raise Exception(
        ...
        mask = max_ids == cat_id
        print(f'interest area coverage {np.sum(mask)}/{len(mask)}')
        return mask
    ...
    def customize_obstacle_map(
        ...
        print("get_dynamic_obstacles_map_3d done")
...
# )
# def create_vlmap(config: DictConfig) -> None:
#     vlmap = VLMap(config)
#     vlmap.create_map('/home/lingkun/Documents/nhrecon-server/data/shanlanlou')


def create_vlmap(map_path: str, config: DictConfig) -> None:
    vlmap = VLMap(config)
    vlmap.create_map(map_path)
...
if __name__ == "__main__":
    ...
    create_vlmap(map_path='/home/lingkun/Documents/nhrecon-server/data/shanlanlou', config=config)
