...
def cvt_pose_vec2tf(pos_quat_vec: np.ndarray) -> np.ndarray:
    ...
    """
    pose_tf = np.eye(4)
    rotation_wc = R.inv(R.from_quat(pos_quat_vec[3:]))
    pose_tf[:3, 3] = np.dot(-rotation_wc.as_matrix(), np.array(pos_quat_vec[:3]).T)
    pose_tf[:3, :3] = rotation_wc.as_matrix()
    return pose_tf
...
def get_scale_offset_inv_depth(sfm_image, sfm_points3d, depth, rgb_w, rgb_h):
    pts_idx = sfm_image.point3D_ids

    mask = pts_idx >= 0
    mask *= pts_idx < len(sfm_points3d)

    pts_idx = pts_idx[mask]
    valid_xys = sfm_image.xys[mask]

    if len(pts_idx) > 0:
        pts = sfm_points3d[pts_idx]
    else:
        pts = np.array([0, 0, 0])
    ...
    valid = (
            ...
            (maps[..., 1] >= 0) *
            (maps[..., 0] < rgb_w * s) *
            (maps[..., 1] < rgb_h * s) * (invcolmapdepth > 0))

    if valid.sum() > 10 and (invcolmapdepth.max() - invcolmapdepth.min()) > 1e-3:
        maps = maps[valid, :]
        invcolmapdepth = invcolmapdepth[valid]
        invmonodepth = cv2.remap(depth, maps[..., 0], maps[..., 1], interpolation=cv2.INTER_LINEAR,
...
def align_scale_offset(depth, sfm_image, sfm_points3d, rgb_w, rgb_h, use_ransac=True):
    pts_idx = sfm_image.point3D_ids
    ...
    valid = (
            ...
            (maps[..., 1] >= 0) *
            (maps[..., 0] < rgb_w * s) *
            (maps[..., 1] < rgb_h * s) * (colmap_depth > 0))

    if valid.sum() < 10:    # todo: or colmap_depth.max() - colmap_depth.min() < 0.5
        return None
    maps = maps[valid, :]
    colmap_depth = colmap_depth[valid]
    mono_detph = cv2.remap(depth, maps[..., 0], maps[..., 1], interpolation=cv2.INTER_LINEAR,
                             borderMode=cv2.BORDER_REPLICATE)[..., 0]
    # mono_detph = 1. / mono_detph
    total_valid_depth = len(colmap_depth)

    if not use_ransac:
        # 法一
        t_colmap = np.median(colmap_depth)
        s_colmap = np.mean(np.abs(colmap_depth - t_colmap))

        t_mono = np.median(mono_detph)
        s_mono = np.mean(np.abs(mono_detph - t_mono))
        scale = s_colmap / s_mono
        offset = t_colmap - t_mono * scale
    else:
        # 法二
        ...
        ransac.fit(mono_detph.reshape(-1, 1), colmap_depth)

        # 提取 scale 和 offset
        scale = ransac.estimator_.coef_[0]  # 回归系数对应 scale
        offset = ransac.estimator_.intercept_  # 截距对应 offset
        inliers = np.sum(ransac.inlier_mask_)
        # print(f'image {sfm_image.name}: depth align inliers {inliers}/{total_valid_depth}')

        # import matplotlib.pyplot as plt
        # plt.figure(figsize=(10, 6))
        # # 绘制原始数据点
        # plt.scatter(mono_detph, colmap_depth, color='blue', label='Original Data', alpha=0.6)
        # # 绘制内点
        # plt.scatter(mono_detph[ransac.inlier_mask_], colmap_depth[ransac.inlier_mask_],
        #             color='green', label='Inliers', alpha=0.6)
        # # 绘制 RANSAC 拟合线
        # x_range = np.linspace(min(mono_detph), max(mono_detph), 500).reshape(-1, 1)
        # y_pred = ransac.predict(x_range)
        # plt.plot(x_range, y_pred, color='red', label=f'RANSAC Fit\n(scale={scale:.2f}, offset={offset:.2f})')
        # # 设置图例和标题
        # plt.title('RANSAC Depth Alignment')
        # plt.xlabel('Mono Depth')
        # plt.ylabel('COLMAP Depth')
        # plt.legend()
        # plt.grid(True)
        # plt.show()

        if inliers / total_valid_depth < 0.8:
            print(f'image {sfm_image.name}: depth align inliers {inliers}/{total_valid_depth} < 0.9, skip!')
            return None
    print(f'image {sfm_image.name} applying scale {scale}, offset {offset}')
    return 1. / (depth * scale + offset)