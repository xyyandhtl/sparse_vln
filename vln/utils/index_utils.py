...
from .clip_utils import get_text_feats, multiple_templates
...
def find_similar_category_id(class_name, classes_list):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"Just choose a element with no other words: What is {class_name} most relevant to among {classes_list_str}"
            }
        ],
        max_tokens=300,
    )
    ...
    print(f'input: What is {class_name} most relevant to among {classes_list_str}\nresponse text: {text}')
